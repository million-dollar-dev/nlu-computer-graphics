...
//		rotate	the	first	ring	about	the	Y	axis	to	get	the	other	rings
		for (int ring = 1; ring < prec + 1; ring++) {
			for (int vert = 0; vert < prec + 1; vert++) { // rotate the vertex positions of the original ring around the
															// Y axis
		...
		// calculate triangle indices corresponding to the two triangles built per
		// vertex
...
//  accessors for the torus indices and vertices
	...
	public Vector3f[] getStangents() {
		return sTangents;
	}

	public Vector3f[] getTtangents() {
		return tTangents;
	}
