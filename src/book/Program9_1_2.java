...
public class Program9_1_2 extends JFrame implements GLEventListener {
	...
	public void init(GLAutoDrawable drawable) {
		...
		skyboxProgram = Utils.createShaderProgram("vertShader9_1.glsl", "fragShader9_1.glsl");
		setupVertices();
		// Camera position
		cameraX = 0.0f;
		cameraY = 2.0f;
	...
	private void renderSkybox(GL4 gl) {
		...
		// Kích hoạt texture của skybox
		gl.glActiveTexture(GL_TEXTURE0);
		gl.glBindTexture(GL_TEXTURE_2D, skyboxTexture);
		gl.glFrontFace(GL_CCW);

		// Tắt depth test để vẽ skybox trước
		gl.glDisable(GL_DEPTH_TEST);
		gl.glDrawArrays(GL_TRIANGLES, 0, 36);
		gl.glEnable(GL_DEPTH_TEST); // Bật lại depth test
	}
